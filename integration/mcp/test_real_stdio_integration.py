...
from openmas.communication.base import get_communicator_class, register_communicator
...
async def create_test_communicator(script_path, agent_name="test_communicator"):
    ...
    try:
        ...
        await safe_async_operation(communicator._connect_to_service, "stdio_server", timeout=CONNECTION_TIMEOUT)
...
async def test_direct_stdio_process() -> None:
    ...
    try:
        ...
        init_request = {
            ...
            "id": "init-1",
            "method": "initialize",
            "params": {"client_info": {"name": "test"}, "capabilities": {}},
        }
...
async def test_agent_with_direct_communicator() -> None:
    ...
    async with create_test_agent(server_script_path, agent_name=unique_agent_name) as agent:
        ...
        communicator = next((c for c in agent._communicators if isinstance(c, McpStdioCommunicator)), None)
