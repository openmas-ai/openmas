...
async def test_simple_mcp_integration() -> None:
    ...
    try:
        ...
        assert (
            result.returncode == 0
        ), f"Server script test-only mode failed with code {result.returncode}. Stderr: {result.stderr}"
    ...
    try:
        ...
        while time.time() - start_time < init_timeout:
            ...
            if server_process.poll() is not None:
                ...
                pytest.fail(
                    f"Server process terminated unexpectedly with code {server_process.returncode}. "
        ...
        if not initialized:
            if server_process.poll() is not None:
                stderr_output = server_process.stderr.read() if server_process.stderr else b"No stderr available"
                pytest.fail(
                    f"Server failed to start. Exit code: {server_process.returncode}. "
                    f"Stderr: {stderr_output.decode('utf-8', errors='replace')}"
                )
            else:
                ...
                stderr_output = server_process.stderr.read() if server_process.stderr else b"No stderr available"
                pytest.fail(
                    f"Server initialization timed out after {init_timeout} seconds. "
                    f"Stderr: {stderr_output.decode('utf-8', errors='replace')}"
                )
            ...
            async with client_manager as manager:
                ...
                try:
                    ...
                    try:
                        ...
                        echo_task = asyncio.create_task(session.call_tool("echo", {"message": message}))
                        echo_result = await asyncio.wait_for(echo_task, timeout=echo_timeout)

                        # Verify the echo result
                        assert (
                            echo_result.get("echoed") == message
                        ), f"Echo result {echo_result} doesn't match input {message}"
                    except asyncio.TimeoutError:
