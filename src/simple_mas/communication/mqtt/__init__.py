"""MQTT communication module for SimpleMAS.

This module provides MQTT-based communication capabilities for SimpleMAS agents.
...
if TYPE_CHECKING:
    from simple_mas.communication.mqtt.communicator import MqttCommunicator
...
def __getattr__(name: str) -> Any:
    ...
    if name == "MqttCommunicator":
        try:
            ...
            raise ImportError(
                "MQTT dependencies are not installed. " "Please install with 'pip install simple-mas[mqtt]'"
            ) from e

    raise AttributeError(f"module 'simple_mas.communication.mqtt' has no attribute '{name}'")
